...
import { Bus, FileText, Pencil, Plus, Printer, Receipt, Trash2 } from "lucide-react";
...
function ClientsPage() {
  ...
  return (
    <AppShell>
      <PageHeader title="كشف حساب ومطالبات الشركات" subtitle="إدارة الشركات العميلة وكشوف الحساب الشهرية" />
      ...
      <Tabs defaultValue="statement">
        <TabsList className="no-print mb-4 border border-border bg-secondary/50">
          ...
          <TabsTrigger value="clients">إدارة الشركات</TabsTrigger>
        </TabsList>
          ...
          {selectedClient && (
            <>
              ...
              <Panel title={`كشف حساب: ${selectedClient}`} className="mt-4">
                ...
                />
                <DataTable head={["التاريخ", "رقم الأتوبيس", "اسم السائق", "الخط / الوردية", "القيمة (دائن)"]}>
                  {invoiceLoading ? (
                    <tr>
                    ...
                    <tr>
                      <td colSpan={5} className="px-4 py-6 text-center text-muted-foreground">لا توجد ورديات مسجلة لهذه الشركة هذا الشهر.</td>
                    </tr>
                  ) : (
                    invoiceRows.map((r, i) => (
